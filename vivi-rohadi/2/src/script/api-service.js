        ...
        commentEl.innerHTML = `
          <div class="flex items-start gap-2.5 my-2 mr-5">
            <div class="w-8 h-8 rounded-full bg-white text-center">
                <h1 class="font-bold text-primary200 p-1 text-xl font-cinzeldecorative">${comment.name.charAt(0)}</h1>
            </div>
            <div class="flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 bg-white rounded-e-xl rounded-es-xl">
                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                    <span class="text-sm font-semibold text-primary200">${comment.name}</span>
                    <span class="text-xs font-semibold ${
                      comment.presense === "hadir" ? "text-green-600" : "text-red-600"
                    }">
                      ${comment.presense === "hadir" ? "Hadir" : "Tidak Hadir"}
                    </span>
                </div>
                <p class="text-sm font-normal py-2.5 text-gray-900">${comment.comment_text}</p>
